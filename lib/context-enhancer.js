...
function isDigitalTwinProfile(profile) {
  ...
  console.log(`🔍 Profile ${profile.profile?.id}: Digital Twin = ${isDigitalTwin} (score: ${score}/${totalChecks})`);
...
export function buildContextualPrompt(prompt, profile, options = {}) {
  ...
  if (result.length > smartMaxLength) {
    console.log(`⚠️ Context prompt too long (${result.length}), intelligently truncating to ${smartMaxLength}`);
