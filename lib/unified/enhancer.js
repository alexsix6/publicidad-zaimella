...
export class UnifiedEnhancer {
  ...
  async initialize() {
    if (!this.initialized) {
      ...
      console.log('[INFO] UnifiedEnhancer initialized');
    }
  }
  ...
  async enhancePrompt(prompt, options = {}) {
    ...
    console.log(`[INFO] [UnifiedEnhancer] Starting enhancement for ${type}`);
    console.log('[INFO] [DEBUG] UnifiedEnhancer received:');
    ...
    try {
      ...
      if (finalProfileId) {
        ...
        console.log(`[INFO] Digital Twin profile validated: ${finalProfileId}`);
      }

      // PASO 2: Aplicar Context Profile si está disponible
      if (finalProfileId) {
        console.log(`[INFO] Applying context profile: ${finalProfileId}`);
        ...
        console.log(`[INFO] Context options for ${isDigitalTwin ? 'Digital Twin' : 'Standard'} profile:`, contextOptions);
      ...
      if (useSanitizer && !isDigitalTwin) {
        console.log('[INFO] Applying prompt sanitization...');
        ...
        try {
          ...
          if (sanitizationResult && sanitizationResult.changed) {
            ...
            replacements.forEach(change => {
              console.log(`[INFO] Sanitized: "${change.term}" → "${change.replacement}" (${change.count} occurrences)`);
            });
            
            result.steps.push(`Prompt sanitized: ${replacements.length} terms replaced`);
            console.log(`[INFO] Sanitized: "${result.enhancedPrompt}"`);
          } else {
            result.steps.push('No sanitization needed');
            console.log('[INFO] No sensitive terms found');
          }
      ...
      if (useAI && !isDigitalTwin) {
        ...
        if (aiResult.success) {
          ...
          console.warn(`[WARN] AI enhancement failed: ${aiResult.error}`);
        ...
        console.log(`[INFO] Skipping AI enhancement for Digital Twin - preserving context profile integrity`);
      ...
      if (useAI && result.aiEnhanced && useSanitizer && !isDigitalTwin) {
        console.log('[INFO] Re-sanitizing after AI enhancement...');
        ...
        try {
          ...
          if (postAISanitization && postAISanitization.changed) {
            ...
            console.log(`[INFO] Re-sanitized: "${result.enhancedPrompt}"`);
            ...
            if (postReplacements.length > 0) {
              ...
              postReplacements.forEach(change => {
                console.log(`[INFO] Post-AI sanitized: "${change.term}" → "${change.replacement}"`);
              });
            }
          } else {
            result.steps.push('No post-AI sanitization needed');
            console.log('[INFO] AI output was clean');
          }
      ...
      if (result.enhancedPrompt && result.enhancedPrompt.length > 0) {
        ...
        console.log('[INFO] UNIFIED ENHANCER METRICS:');
      ...
      console.error('[ERROR] UnifiedEnhancer error:', error);
    ...
    try {
      ...
      if (hasProductKeywords) {
        ...
        for (const profile of profiles) {
          ...
          if (isDigitalTwin) {
            ...
            if (hasMatch) {
              console.log(`[INFO] Auto-selected Digital Twin profile: ${profile.id} (product match)`);
              return profile.id;
            }
          }
        }
      ...
      for (const [category, keywords] of Object.entries(keywordMatches)) {
        if (keywords.some(keyword => promptLower.includes(keyword))) {
          ...
          if (matchingProfile) {
            console.log(`[INFO] Auto-selected profile: ${matchingProfile.id} (${category} match)`);
      ...
      console.log(`[INFO] Auto-selected most used profile: ${mostUsedProfile.id}`);
  ...
  async recordProfileSuccess(profileId, originalPrompt, enhancedPrompt, isDigitalTwin = false) {
    try {
      ...
      console.error('[ERROR] Error recording profile success:', error);
    }
  }
