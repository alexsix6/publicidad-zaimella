...
export async function generateVideoWithVeo3(prompt, imageUrl = null, aspectRatio = "16:9", duration = "8s") {
  try {
    ...
    // FAL.ai Model Selection: Standard ($0.75/seg con audio) vs Fast ($0.40/seg con audio)
    const modelType = process.env.VEO3_MODEL_TYPE || 'fast'; // 🆕 DEFAULT: 'fast' para costo optimizado
    
    const requestBody = {
      prompt: finalPrompt,
      // Configuración de modelo FAL específica
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
      ...
      if (statusResult.status === 'COMPLETED') {
        ...
        // Calcular costo estimado basado en modelo FAL
        const costPerSecond = modelType === 'fast' ? 0.40 : 0.75; // USD con audio
        const estimatedCost = durationNumber * costPerSecond;
        ...
        return {
          ...
          costBreakdown: `${durationNumber}s × $${costPerSecond} = $${estimatedCost.toFixed(2)} USD`
        };
